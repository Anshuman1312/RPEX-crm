...
const MENU_ITEMS: MenuItem[] = [
  ...
  },
  {
    ...
    path: "/campaigns",
    group: "crm",
    roles: ["ADMIN", "SEO_MANAGER", "SUPER_ADMIN", "MARKETING_MANAGER"],
  },
  {
    label: "Projects",
    path: "/projects",
  ...
  },
  {
    ...
    path: "/finance",
    group: "crm",
    roles: ["ADMIN", "SUPER_ADMIN", "FINANCE", "DIRECTOR"],
  },
  {
    label: "Documents",
    path: "/documents",
    group: "crm",
    roles: ["ADMIN", "SUPER_ADMIN", "LEGAL", "CRM_EXECUTIVE"],
  },
  {
    label: "Reports",
    path: "/reports",
    group: "crm",
    roles: ["ADMIN", "ANALYST", "SUPER_ADMIN", "DIRECTOR", "FINANCE", "HR"],
  },
  {
    label: "Partner Portal",
    path: "/partner",
    group: "partner",
    roles: ["CHANNEL_PARTNER", "CUSTOMER_PORTAL", "ADMIN", "SUPER_ADMIN"],
  },
];
...
export function AppShell() {
  ...
  return (
    <div className="min-h-screen crm-grid">
      <aside
        className={`sidebar fixed inset-y-0 left-0 z-40 flex w-[280px] flex-col p-5 transition-transform duration-300 md:sticky md:top-0 md:h-screen md:translate-x-0 md:p-6 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        <div>
          <div className="flex items-center justify-between gap-3">
            <div>
              <h1 className="font-display text-2xl text-slate-50">
                {isPartnerMode ? "RPEX Partner" : "RPEX CRM"}
              </h1>
              <p className="text-slate-300 text-sm mt-1">
                {isPartnerMode
                  ? "Partner bookings and payout workspace"
                  : "SEO Inquiry Operations"}
              </p>
            </div>
            <button
            ...
            <div className="mt-6 space-y-2">
              <div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 p-3">
                ...
                <p className="mt-1 text-sm text-slate-200">
                  Dedicated portal for bookings, collections, documents, and
                  payout visibility.
                </p>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="truncate text-xs text-slate-400">
                {profile?.email ?? "profile syncing..."}
              </p>
              <p className="mt-1 text-[11px] uppercase tracking-[0.18em] text-cyan-300">
                {profile?.role ?? role ?? "Unknown role"}
              </p>
              <p className="mt-1 text-xs text-slate-400">
                {profile?.role_description ?? "System access profile"}
              </p>
            </div>
          </div>
