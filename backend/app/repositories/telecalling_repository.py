...
class TelecallingRepository:
    ...
    async def daily_summary(self, for_date: date):
        stmt = (
            select(
                TelecallingCall.telecaller_id,
                func.count(TelecallingCall.id).label("daily_calls"),
                func.sum(func.case((TelecallingCall.status == "CONNECTED", 1), else_=0)).label("connected"),
                func.sum(func.case((TelecallingCall.status == "NOT_CONNECTED", 1), else_=0)).label("not_connected"),
                func.sum(func.case((TelecallingCall.status == "INTERESTED", 1), else_=0)).label("interested"),
            ...
            .where(TelecallingCall.call_date == for_date)
