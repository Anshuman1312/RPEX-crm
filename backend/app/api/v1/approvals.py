...
@router.post("", dependencies=[Depends(require_permissions("manage_finance"))])
async def create_approval_request(payload: ApprovalRequestCreate, current_user: CurrentUser, db: AsyncSession = Depends(get_db)):
    service = ApprovalService(ApprovalRepository(db), AuditLogRepository(db))
...
@router.get("", dependencies=[Depends(require_permissions("manage_finance"))])
...
@router.post("/{request_id}/decision", dependencies=[Depends(require_permissions("manage_finance"))])
