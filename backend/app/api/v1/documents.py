...
async def upload_document(
    ...
    if not getattr(settings, "cloudinary_cloud_name", None):
        raise HTTPException(
            status_code=status.HTTP_53_SERVICE_UNAVAILABLE if hasattr(status, "HTTP_53_SERVICE_UNAVAILABLE") else 503,
    ...
    upload_result = upload_bytes(
        ...
        public_id=public_id,
        folder=getattr(settings, "cloudinary_folder", "rpex_documents"),
    )
