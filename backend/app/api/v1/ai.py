...
@router.post("/ask", dependencies=[Depends(require_permissions("view_analytics"))])
async def ask_ai(payload: AIAskRequest, _: CurrentUser, db: AsyncSession = Depends(get_db)):
    ...
    if "closed the most deals" in q:
        ...
        rows = (
            await db.execute(
                select(user_name_col, func.count(Booking.id).label("bookings"))
                .join(Booking, Booking.sales_executive_id == User.id)
                .group_by(user_name_col)
