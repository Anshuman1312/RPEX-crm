...
@router.get("/dashboard", dependencies=[Depends(require_permissions("view_analytics"))])
async def dashboard_metrics(_: CurrentUser, db: AsyncSession = Depends(get_db)):
    ...
    if user_ids:
        user_name_col = getattr(User, "name", getattr(User, "full_name", User.email))
        users = (
            await db.execute(select(User.id, user_name_col).where(User.id.in_(list(user_ids))))
        ).all()
        for user in users:
            key = str(user.id)
            if key in team_stats:
                team_stats[key]["name"] = user[1]
